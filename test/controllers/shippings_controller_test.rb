...
class ShippingsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create shipping" do
    assert_difference("Shipping.count") do
      post shippings_url, params: { shipping: { name: @shipping.name, status: @shipping.status, value: @shipping.value } }
    end
  ...
  test "should update shipping" do
    patch shipping_url(@shipping), params: { shipping: { name: @shipping.name, status: @shipping.status, value: @shipping.value } }
